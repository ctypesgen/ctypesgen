...
# This function looks for functions that might be grammar rules, but which don't have the proper
# p_suffix.
def validate_dict(d):
    for n, v in d.items():
                                ...
                                if r > 0:
                                    ...
                                    if (slevel < rlevel) or (
                                        ...
                                        if not slevel and not rlevel:
                                            _vfc.write(
                                                "shift/reduce conflict in state %d resolved as "
                                                "reduce.\n" % st
                                            )
                                            _vf.write(
...
def yacc(
    ...
    if optimize and lr_read_tables(tabmodule, 1):
        ...
        if not (
            isinstance(tokens, types.ListType) or isinstance(tokens, types.TupleType)
        ):
        ...
        if requires:
            ...
            for r, v in requires.items():
                try:
                    ...
                    print(
                        "Invalid specification for rule '%s' in require."
                        " Expected a list of strings" % r
                    )
        ...
        if prec:
            if not (
                isinstance(prec, types.ListType) or isinstance(prec, types.TupleType)
            ):
        ...
        symbols = [
            ...
            for f in ldict.keys()
            if (
                type(ldict[f]) in (types.FunctionType, types.MethodType)
                and ldict[f].__name__[:2] == "p_"
        ...
        symbols.sort(
            lambda x, y: cmp(x.func_code.co_firstlineno, y.func_code.co_firstlineno)
        )
        ...
        if not lr_read_tables(tabmodule):
            ...
            otherfunc = [
                ...
                for f in ldict.keys()
                if (
                    type(f) in (types.FunctionType, types.MethodType)
                    and ldict[f].__name__[:2] != "p_"
                )
            ]
