...
class WrapperPrinter:
    def __init__(self,outpath,options,data):
        ...
        filter=lambda list: [x for x in list if x.included]
        ...
        self.print_group(filter(data.constants),"constants",self.print_constant)
        self.print_group(filter(data.structs),"structs",self.print_struct)
        self.print_group(filter(data.enums),"enums",self.print_enum)
        self.print_group(filter(data.typedefs),"typedefs",self.print_typedef)
        self.print_group([s for s in filter(data.structs) if not s.opaque],
                         "struct bodies", self.print_struct_members)
        self.print_group(filter(data.functions),"functions",self.print_function)
        self.print_group(filter(data.variables),"variables",self.print_variable)
        self.print_group(filter(data.macros),"macros",self.print_macro)
    ...
    def print_simple_macro(self, macro):
        # The macro translator makes heroic efforts but it sometimes fails.
