...
class MyGenerator:
    def __init__(self, given_args=None):
        if given_args is not None:
            self.givenArgs = given_args
        else:
            self.givenArgs = None

    # noinspection PyMethodMayBeStatic
    def find_names_in_modules(self, modules):
        names = set()
    ...
    def main(self):
        ...
        args = parser.parse_args(self.givenArgs)
        ...
        args.other_known_names = self.find_names_in_modules(args.modules)
        ...
        # Step 1: Parse
        descriptions = MyParser(args.headers, args)

        # Step 2: Process
        MyProcessor(args, descriptions).process()

        # Step 3: Print
        MyPrinter(args.output, args, descriptions)
